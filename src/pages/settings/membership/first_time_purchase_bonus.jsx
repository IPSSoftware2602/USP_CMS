...
const FirstTimePurchaseBonus = () => {
  const [voucherForm, setVoucherForm] = useState({
    rows: [],
    bonus: 0,
    status: 'Inactive',
    type: 'first_time_purchase_bonus',
  });
  ...
  const fetchMembershipSettings = async () => {
    try {
      const response = await membershipSettingService.get('first_time_purchase_bonus');
      const parsedSetting = (() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/settings/membership/first_time_purchase_bonus');
    ...
    <div className='rounded-lg shadow-lg bg-white max-w-3xl mx-auto mt-10'>
      <div className="mx-auto py-3 px-4 sm:px-6 lg:px-8">
        <div className="p-3 pb-2 pt-5 flex justify-between items-center">
            <h2 className="text-xl font-medium text-gray-800">First Time Purchase Bonus</h2>
...
export default FirstTimePurchaseBonus;
