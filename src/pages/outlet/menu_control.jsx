...
import { Check, X, ChevronDown, ChevronRight, Loader2, Layers, List } from 'lucide-react';
                  ...
                  <div key={outlet.id} onClick={() => toggleOutletSelection(outlet.id)} className={`p-2 border cursor-pointer transition-colors ${selectedOutlets.includes(outlet.id) ? 'bg-indigo-50 border-indigo-300' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                    <div className="flex items-center">
                      ...
                      <div className="truncate ml-2">
                        <p className="text-xs font-medium text-gray-900 truncate">{(outlet.title || outlet.name || '').replace(/uspizza|Auntea Jenny/gi, '').replace(/-/g, '').trim()}</p>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                {categories.map(category => {
                  ...
                  return (
                    <div key={category.id} className="border-b last:border-b-0">
                      ...
                      >
                        <span className="font-medium text-gray-900 flex-1">{category.title} ({categoryItems.length} items)</span>
