...
const OrderComfirmed = () => {
  ...
  const loadOrders = async (page = 1, perPage = 100) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const transformed = orders.map((order) => ({
        ...
        created_at: order.created_at, // Keep original created date
        updated_at: order.updated_at || order.created_at, // Fallback to created if updated doesn't exist
      ...
      setData(transformed);
      setFilteredData(transformed); // 👈 ensures table renders immediately
      ...
      if (filters.search && filters.search.length >= 3) {
        ...
        filtered = filtered.filter(
          (order) =>
            ...
            order.orderSO.toLowerCase().includes(searchTerm) ||
